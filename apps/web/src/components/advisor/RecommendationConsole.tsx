...
type Props = {
  ...
  onStartRun: () => Promise<{ id: string; events: string[] }>;
  onApproveRun: () => Promise<void>;
  onAsk: (question: string) => Promise<string>;
  onDraftQuote: (serviceCodes: string[]) => Promise<QuoteDraftResponse>;
  ...
  return (
    <Card className="mt-6 border border-white/10 bg-[#20201c]/95 py-0 shadow-2xl shadow-black/20">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="mb-1 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary">
              <BookOpenCheck className="size-3.5" /> 02 · Evidence review
            </p>
            <CardTitle className="text-xl">
              Grounded maintenance recommendation
            </CardTitle>
            <p className="mt-1 text-sm text-muted-foreground">
              A recommendation cannot outpace its reviewed source.
            </p>
          </div>
          <Badge
            className="h-auto max-w-64 whitespace-normal bg-primary/15 px-3 py-1.5 text-center text-primary"
            variant="outline"
          >
            {state}
          </Badge>
      ...
      <CardContent className="space-y-5 px-5 py-5 sm:px-6">
        <Alert className="border-primary/25 bg-primary/7">
          <ShieldCheck className="size-4 text-primary" />
          <AlertTitle>
            {recommendation?.service_code ?? "No actionable service"}
          </AlertTitle>
          <AlertDescription>
            {recommendation
              ? `${recommendation.rule_version} · page ${recommendation.citation_page}, ${recommendation.citation_section}.`
              : "Confirm a check-in to load reviewed evidence."}
          </AlertDescription>
        </Alert>
        <Tabs defaultValue="evidence" className="space-y-4">
          ...
          >
            ...
            {recommendation?.due_reason ?? "No recommendation yet."}
          </TabsContent>
          <TabsContent
            value="run"
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Button
                onClick={() =>
                  void onStartRun().then((run) =>
                    setRunState(`Run ${run.id}: ${run.events.join(" → ")}`),
                  )
                }
              >
                Start run
              ...
              <Button
                variant="outline"
                onClick={() =>
                  void onApproveRun().then(() =>
                    setRunState("Approved idempotently"),
                  )
                }
              >
                Approve review
          ...
          >
            <label
              ...
              htmlFor="contextual-question"
            >
              Ask about this citation
            </label>
            <Textarea
              id="contextual-question"
              aria-label="Contextual question"
              value={question}
              onChange={(event) => setQuestion(event.target.value)}
              placeholder="Ask about this cited recommendation"
            />
            <Button
              className="mt-3"
              onClick={() => void onAsk(question).then(setAnswer)}
            >
              Ask
            </Button>
            <p className="mt-3 text-sm text-muted-foreground">
              {answer ||
                "Answers remain grounded in the displayed rule and evidence."}
            </p>
          </TabsContent>
          <TabsContent
            value="quote"
