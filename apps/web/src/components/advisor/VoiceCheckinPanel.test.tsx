...
const TRANSCRIBED = {
  ...
  segments: [
    ...
    { starts_at_seconds: 6.5, text: 'Pide revision antes del viaje del viernes' },
  ],
...
function renderPanel(overrides: Record<string, unknown> = {}) {
  ...
  render(<VoiceCheckinPanel {...(props as unknown as Parameters<typeof VoiceCheckinPanel>[0])} />)
  return props
}
...
test('refuses a recording longer than ninety seconds', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Recording seconds'), { target: { value: '120' } })

  fireEvent.click(screen.getByRole('button', { name: 'Transcribe voice note' }))

  expect(await screen.findByText('A voice note may not exceed 90 seconds')).toBeVisible()
...
test('reports the confirmed transcript and deleted audio', async () => {
  ...
  expect(await screen.findByText('Transcript confirmed and audio deleted')).toBeVisible()
...
test('keeps manual entry available after a provider failure', async () => {
  ...
  expect(
    await screen.findByText('Transcription provider is unavailable; enter the concern manually'),
  ).toBeVisible()
